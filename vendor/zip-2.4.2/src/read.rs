...
impl<'a> TryFrom<&'a CentralDirectoryEndInfo> for CentralDirectoryInfo {
    ...
    fn try_from(value: &'a CentralDirectoryEndInfo) -> Result<Self, Self::Error> {
        let (
            ...
            _ => {
                if value.eocd.data.number_of_files_on_this_disk > value.eocd.data.number_of_files {
                    return Err(InvalidArchive(
                        "ZIP footer indicates more files on this disk than in the whole archive",
                    ));
                }
                (
                    value.eocd.data.central_directory_offset as u64,
                    value.eocd.data.central_directory_size as u64,
                    value.eocd.data.number_of_files as u64,
        ...
        let directory_end = directory_start
            ...
            .ok_or(InvalidArchive("Invalid central directory size or offset"))?;
        if directory_end > footer_position {
            return Err(InvalidArchive(
...
impl<R: Read + Seek> ZipArchive<R> {
    ...
    pub(crate) fn get_metadata(
        ...
        loop {
            ...
            let cde = spec::find_central_directory(
                ...
                end_exclusive,
                file_len,
                limits.max_zip64_eocd_size,
            )?;

            // Turn EOCD into internal representation.
            let Ok(shared) = CentralDirectoryInfo::try_from(&cde)
                .and_then(|info| Self::read_central_header(info, config, limits, reader))
            else {
    ...
    fn read_central_header(
        ...
        if limits
            .max_entries
            .is_some_and(|limit| dir_info.number_of_files > limit)
        {
            return unsupported_zip_error("ZIP entry count exceeds configured limit");
        }
        if limits
            .max_central_directory_size
            .is_some_and(|limit| dir_info.directory_size > limit)
        {
            return unsupported_zip_error("ZIP central directory exceeds configured limit");
        }
        let minimum_directory_size = dir_info
            .number_of_files
            .checked_mul(size_of::<ZipCentralEntryBlock>() as u64)
            .ok_or(InvalidArchive("Invalid central directory entry count"))?;
        if minimum_directory_size > dir_info.directory_size {
            return Err(InvalidArchive(
                "Central directory is too small for its declared entry count",
            ));
        }
        let number_of_files = usize::try_from(dir_info.number_of_files)
            .map_err(|_| ZipError::UnsupportedArchive("Too many ZIP entries for this platform"))?;
        // If the parsed number of files is greater than the offset then
        // something fishy is going on and we shouldn't trust number_of_files.
        let file_capacity = if dir_info.number_of_files > dir_info.directory_start {
            0
        } else {
            number_of_files
        };
        ...
        files
            .try_reserve_exact(file_capacity)
            .map_err(|_| ZipError::UnsupportedArchive("ZIP metadata allocation failed"))?;
    ...
    pub fn with_config(config: Config, reader: R) -> ZipResult<ZipArchive<R>> {
        Self::with_config_and_limits(config, reader, ReadLimits::default())
    }

    /// Read a ZIP archive while applying resource limits to its metadata.
    pub fn with_limits(reader: R, limits: ReadLimits) -> ZipResult<ZipArchive<R>> {
        Self::with_config_and_limits(Config::default(), reader, limits)
    }

    /// Read a ZIP archive with both offset configuration and metadata limits.
    pub fn with_config_and_limits(
        config: Config,
        mut reader: R,
        limits: ReadLimits,
    ) -> ZipResult<ZipArchive<R>> {
        let shared = Self::get_metadata(config, limits, &mut reader)?;
...
pub(crate) fn central_header_to_zip_file<R: Read + Seek>(
    ...
    let fixed_header_end = central_header_start
        .checked_add(size_of::<ZipCentralEntryBlock>() as u64)
        .ok_or(InvalidArchive("Central directory entry size overflow"))?;
    if fixed_header_end > central_directory.directory_end {
        return Err(InvalidArchive(
            "Central directory entry exceeds the declared directory size",
        ));
    }

    // Parse central header
    let block = ZipCentralEntryBlock::parse(reader)?;
    let variable_size = u64::from(block.file_name_length)
        .checked_add(u64::from(block.extra_field_length))
        .and_then(|size| size.checked_add(u64::from(block.file_comment_length)))
        .ok_or(InvalidArchive("Central directory entry size overflow"))?;
    let declared_header_end = fixed_header_end
        .checked_add(variable_size)
        .ok_or(InvalidArchive("Central directory entry size overflow"))?;
    if declared_header_end > central_directory.directory_end {
        return Err(InvalidArchive(
            "Central directory entry exceeds the declared directory size",
        ));
    }
