...
impl Zip64CentralDirectoryEnd {
    pub fn parse<T: Read>(
        reader: &mut T,
        max_size: u64,
        configured_max_size: Option<u64>,
        ...
        let total_size = record_size
            .checked_add(12)
            .ok_or(ZipError::InvalidArchive("EOCD64 record size overflow"))?;
        if configured_max_size.is_some_and(|limit| total_size > limit) {
            return Err(ZipError::UnsupportedArchive(
                "ZIP64 end record exceeds configured limit",
            ));
        }

        let extensible_size = usize::try_from(record_size - 44).map_err(|_| {
            ZipError::UnsupportedArchive("ZIP64 end record is too large for this platform")
        })?;
        let mut zip_file_comment = Vec::new();
        zip_file_comment
            .try_reserve_exact(extensible_size)
            .map_err(|_| ZipError::UnsupportedArchive("ZIP64 end record allocation failed"))?;
        zip_file_comment.resize(extensible_size, 0);
        let mut zip_file_comment = zip_file_comment.into_boxed_slice();
...
pub(crate) fn find_central_directory<R: Read + Seek>(
    ...
    end_exclusive: u64,
    file_len: u64,
    max_zip64_eocd_size: Option<u64>,
    ...
    while let Some(eocd_offset) = eocd_finder.next(reader)? {
        ...
        fn try_read_eocd64<R: Read + Seek>(
            ...
            let z64 =
                Zip64CentralDirectoryEnd::parse(reader, expected_length, configured_max_size)?;
...
mod test {
    ...
    #[test]
    fn zip64_end_record_limit_is_checked_before_extensible_data() {
        let extensible_size = 64_u64;
        let record_size = 44 + extensible_size;
        let total_size = record_size + 12;
        let record = Zip64CentralDirectoryEnd {
            ...
            extensible_data_sector: vec![0_u8; extensible_size as usize].into_boxed_slice(),
        };
        ...
        assert!(matches!(
            Zip64CentralDirectoryEnd::parse(&mut bytes, total_size, Some(total_size - 1)),
            Err(ZipError::UnsupportedArchive(_))
        ));

        bytes.set_position(0);
        let parsed =
            Zip64CentralDirectoryEnd::parse(&mut bytes, total_size, Some(total_size)).unwrap();
        assert_eq!(
            parsed.extensible_data_sector.len(),
            extensible_size as usize
        );
    }
}
